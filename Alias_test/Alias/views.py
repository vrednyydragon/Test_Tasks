...
max_system_date = '9999-12-31 00:00:00.000001+00:00'
...
def get_on_date(request, obj_alias, on_date):
...
def aliases(request, alias_target, from_datetime, to_datetime):
...
def replace(request, existing_alias, alias_start, new_alias_value):
    """ Replacing an existing alias with a new one at a specific time point
    alias_start - date when the active alias started
    ...
    message = save_with_check(new_alias_value, alias_obj[0].target, alias_obj[0].end)
    return HttpResponse(message)


def alias_save(obj_alias, obj_target, obj_start_time, obj_end_time):
...
def save_with_check(obj_alias, obj_target, obj_start_time, obj_end_time=max_system_date):
